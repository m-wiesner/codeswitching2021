...
def main():
    ...
    for w, prons in lex_orig.items():
        for pron in prons:
            new_pron = ''
            w_start = 0
            for match in devanagari.finditer(pron):
                w_start_, w_end = match.span()
                devanagari_word = pron[w_start_:w_end].replace(' ', '')
                if devanagari_word in lex_supp:
                    pron_idx = int((random.random() - 1e-10) * len(prons))   
                    devanagari_pron = lex_supp[devanagari_word][pron_idx]
                else:
                    cmd="phonetisaurus-g2pfst --model={} --word={} --nbest=1".format(args.g2p, devanagari_word)
                    g2p_out = subprocess.check_output(cmd.split(), stderr=subprocess.DEVNULL)
                    devanagari_pron = g2p_out.decode().split('\t')[2]
                devanagari_pron_xsampa = u' '.join(map(xs.ipa2xs, devanagari_pron.split()))
                if args.use_lang_tags:
                    devanagari_pron_xsampa = u' '.join([i + "_L2" for i in devanagari_pron_xsampa.split()])
                pron_xsampa = u' '.join([arpa2xs.get(i, i) for i in pron[w_start:w_start_].split()])
                new_pron += pron_xsampa + ' ' + devanagari_pron_xsampa + ' ' 
                w_start = w_end
