...
stage=-1
subsampling=4
traindir=data/train_cleaned
langdir=data/lang_wikipron
testdir=data/test
...
if [ $stage -le -1 ]; then
  ./utils/data/perturb_data_dir_speed.sh ${traindir} ${traindir}_sp  
  trainname=`basename ${traindir}_sp`
  ./steps/make_mfcc.sh --nj 40 --cmd "$train_cmd" \
    ${traindir}_sp exp/make_mfcc/${trainname} mfcc
  ./steps/compute_cmvn_stats.sh ${traindir}_sp exp/make_mfcc/${trainname} mfcc
  ./utils/fix_data_dir.sh ${traindir}_sp
  
  ./steps/align_fmllr.sh --cmd "$train_cmd" --nj 40 \
    ${traindir}_sp ${langdir} ${src} ${src}_ali_cleaned_sp  
fi

if [ $stage -le 0 ]; then
  for d in ${traindir}_sp ${testdir}; do
    ./utils/copy_data_dir.sh ${d} ${d}${feat_affix}
    if [[ $d = "${traindir}_sp" ]]; then
      ./utils/data/perturb_data_dir_volume.sh ${d}${feat_affix}  
    fi
